...
Namespace CustomGrid_PreviewRow
    ...
    Public Class CustomGridViewInfo
        ...
        Protected Overrides Sub CalcMinBandRowHeight(ByVal bandHeight As Integer)
            If Painter IsNot Nothing Then
                MyBase.CalcMinBandRowHeight(bandHeight)
            End If
        End Sub

        Private fRowPreviewViewInfo As BaseEditViewInfo
        Public Sub New(ByVal gridView As AdvBandedGridView)
            MyBase.New(gridView)
        ...
        Public Overridable Function GetRowPreviewEditBounds(ByVal ri As GridDataRowInfo) As Rectangle
            ...
            Dim _previewTextIndent = Painter.ElementsPainter.RowPreview.GetPreviewTextIndent(ri)
            Dim _previewTextVIndent = Painter.ElementsPainter.RowPreview.GetPreviewTextVIndent(ri)
            r.Inflate(-_previewTextIndent, -_previewTextVIndent)
        ...
        Protected Overridable Function CalcRowPreviewEditorHeight(ByVal rowHandle As Integer, ByVal item As RepositoryItem) As Integer
            ...
            End If
            Dim g As Graphics = GInfo.AddGraphics(Nothing)
            Dim _previewTextIndent As Integer = Painter.ElementsPainter.RowPreview.GetPreviewTextIndent(EmptyDataRow)
            Try
                ...
                If ha IsNot Nothing Then
                    ...
                    res = ha.CalcHeight(GInfo.Cache, Me.CalcRowPreviewWidth(rowHandle) - Me.PreviewIndent - _previewTextIndent * 2)
                End If
                res = Math.Max(fRowPreviewViewInfo.CalcMinHeight(g), res)
            Finally
                GInfo.ReleaseGraphics()
            End Try
            res += _previewTextIndent * 2
            Return res
        End Function
